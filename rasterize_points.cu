	...
	)
{
  ...
  torch::Tensor pixels = torch::zeros({1, H, W}, means3D.options());
  ...
  return std::make_tuple(rendered, out_color, out_feature_map, out_alpha, out_depth, radii, pixels, geomBuffer, binningBuffer, imgBuffer);
}
